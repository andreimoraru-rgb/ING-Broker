export interface Article {
  ...
  /**
   * true pentru articolele produse automat din `rawTopics`, cu titlu si continut
   * generate dupa sablon. Acestea primesc `noindex, follow` in <head>.
   *
   * Motivul: sunt ~190 de pagini cu structura identica si continut sablonat pe
   * acelasi domeniu. Google numeste asta scaled content abuse si sanctioneaza
   * intregul domeniu, nu doar paginile in cauza. Cat timp site-ul era 100% CSR,
   * crawlerii le vedeau greu si riscul era latent. Din momentul in care paginile
   * sunt prerenderizate (vezi scripts/prerender.ts), devin perfect lizibile -
   * deci `noindex` trebuie sa intre in aceeasi livrare cu prerender-ul.
   *
   * `follow` si nu `none`: linkurile interne catre paginile reale raman utile.
   *
   * Articolele scrise manual NU au acest camp si raman indexabile.
   */
