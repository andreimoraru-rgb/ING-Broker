...
export const SEO = ({ title, description, lang, schema, keywords, image = "https://www.ingbroker.md/Logo%20ING%20Broker.png" }: SEOProps) => {
    ...
    // Logic: strip any leading /ru/ or /en/ prefix to get the base path,
    // then reconstruct URLs for ro (no prefix), ru (/ru/), en (/en/).
    const rawPath = location.pathname;
    let basePath: string;
    if (rawPath.startsWith('/ru/')) {
          basePath = rawPath.slice(3); // remove /ru -> keep /rest
    } else if (rawPath.startsWith('/en/')) {
          basePath = rawPath.slice(3); // remove /en -> keep /rest
