...
const selections={rc:{label:"ЗАЩИТА ТРЕТЬИХ ЛИЦ",title:"Ответственность при эксплуатации дрона",copy:"За вред третьим лицам. Ориентировочные тарифы от 500 MDL в год в зависимости от лимита и франшизы. Страхование ответственности не покрывает сам дрон.",image:'risk-injury.webp',alt:'Persoană și dronă într-un spațiu urban, pentru analiza răspunderii față de terți',subject:'Solicitare RC Drone'},casco:{label:"ЗАЩИТА ВАШЕГО ДРОНА",title:'CASCO Drone / AEROCASCO',copy:"AEROCASCO Asterra: ориентировочный тариф 4% страховой суммы, минимум 1 000 MDL в год. Резюме включает повреждение, уничтожение, пропажу без вести и кражу согласно условиям договора.",image:'selector-casco.webp',alt:'Dronă profesională pe masa de inspecție tehnică',subject:'Solicitare CASCO Drone'},both:{label:"ДВА ДОПОЛНЯЮЩИХ ПОКРЫТИЯ",title:"Ответственность + КАСКО",copy:"Мы совместно анализируем ущерб третьим лицам и риски вашего дрона. У каждого покрытия свои условия, лимиты и исключения.",image:'selector-both.webp',alt:'Operator și dronă pregătiți pentru o misiune în teren',subject:'Solicitare RC + CASCO Drone'}};
...
if(radar&&radarToggle){
  ...
  paused=false;
  radarToggle.textContent=paused?"Запустить анимацию":"Остановить анимацию";
  ...
  reduceMotion.addEventListener('change',()=>{paused=false;radar.classList.remove('motion-opt-in');radarToggle.textContent=paused?"Запустить анимацию":"Остановить анимацию";radarToggle.setAttribute('aria-pressed',String(paused));syncRadar()});
  radarToggle.addEventListener('click',()=>{paused=!paused;radar.classList.toggle('motion-opt-in',!paused);radarToggle.setAttribute('aria-pressed',String(paused));radarToggle.textContent=paused?"Запустить анимацию":"Остановить анимацию";syncRadar()});
}
