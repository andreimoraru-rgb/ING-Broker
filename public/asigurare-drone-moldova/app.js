...
const selections={rc:{label:'PROTECȚIE PENTRU TERȚI',title:'Răspundere Civilă Drone',copy:'Pentru prejudiciile produse terților. Tarife indicative de la 500 MDL/an, în funcție de limita și franșiza alese. Propria dronă nu este acoperită prin RC.',image:'risk-injury.webp',alt:'Persoană și dronă într-un spațiu urban, pentru analiza răspunderii față de terți',subject:'Solicitare RC Drone'},casco:{label:'PROTECȚIE PENTRU PROPRIA DRONĂ',title:'CASCO Drone / AEROCASCO',copy:'AEROCASCO Asterra: tarif indicativ de 4% din suma asigurată, minimum 1 000 MDL/an. Rezumatul include avarierea, distrugerea, pierderea fără urmă și furtul, conform condițiilor contractuale.',image:'selector-casco.webp',alt:'Dronă profesională pe masa de inspecție tehnică',subject:'Solicitare CASCO Drone'},both:{label:'DOUĂ ACOPERIRI COMPLEMENTARE',title:'Răspundere Civilă + CASCO',copy:'Analizăm împreună prejudiciile produse terților și riscurile propriei drone. Fiecare tip de protecție își păstrează condițiile, limitele și excluderile.',image:'selector-both.webp',alt:'Operator și dronă pregătiți pentru o misiune în teren',subject:'Solicitare RC + CASCO Drone'}};
...
if(radar&&radarToggle){
  ...
  paused=false;
  radarToggle.textContent=paused?'Pornește animația':'Oprește animația';
  ...
  reduceMotion.addEventListener('change',()=>{paused=false;radar.classList.remove('motion-opt-in');radarToggle.textContent=paused?'Pornește animația':'Oprește animația';radarToggle.setAttribute('aria-pressed',String(paused));syncRadar()});
  radarToggle.addEventListener('click',()=>{paused=!paused;radar.classList.toggle('motion-opt-in',!paused);radarToggle.setAttribute('aria-pressed',String(paused));radarToggle.textContent=paused?'Pornește animația':'Oprește animația';syncRadar()});
}
